...
export const Title = styled.h2<StyledProps>`
  ${tw` mb-4 text-4xl font-normal w-full text-left`};
  ${({ center }) => center && tw`text-center`};
`;

export const SubTitle = styled.h4<StyledProps>`
  ${tw`text-sm text-indigo-600 w-full text-left`};
  ${({ center }) => center && tw`text-center`};
`;

export const Separator = styled.h2<StyledProps>`
  ${tw`relative w-2 h-4 mb-6 -mt-2`};
  ...
  &:after {
    content: '';
    ${tw`bg-teal-400 h-2 w-px absolute ml-1`};
  }
`;
